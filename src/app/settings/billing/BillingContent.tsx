...
export function BillingContent({ initialTier, subscriptionStatus, hasStripeCustomer, paymentCanceled }: BillingContentProps) {
  const [currentTier, setCurrentTier] = useState(initialTier)

  const handleSyncComplete = (newTier: string) => {
  ...
  return (
    <>
      ...
      <AutoSyncSubscription onSyncComplete={handleSyncComplete} paymentCanceled={paymentCanceled} />
      ...
      <div className="page-container" style={{ paddingTop: '40px', paddingBottom: '64px' }}>
        ...
        {/* Plan cards */}
        <div className="profile-stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '4px', marginBottom: '40px' }}>
          {PLANS.map(plan => {
